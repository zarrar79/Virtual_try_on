...
export default function Login() {
  ...
  const handleForgotPassword = async (): Promise<void> => {
    ...
    try {
      ...
      Alert.alert('Success', response.data.msg || 'Password reset email sent');
    } catch (err) {
      const error = err as AxiosError<{ msg: string }>;
      console.error(error.response?.data || error.message);
      Alert.alert('Error', error.response?.data?.msg || 'Failed to send reset email');
    }
  };
              ...
              <View style={styles.overlay}>
                <View style={styles.formWrapper}>
                  ...
                  <Pressable onPress={handleForgotPassword}>
                    <Text style={[styles.toggleText, { marginBottom: 10 }]}>
                      Forgot your password? <Text style={styles.toggleLink}>Reset</Text>
                    </Text>
                  </Pressable>
...
const styles = StyleSheet.create({
  ...
  containerLarge: { flexDirection: 'row' },
  formContainer: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)' },
  leftSide: { flex: 1 },
  ...
  heading: { fontSize: 32, fontWeight: 'bold', color: '#fff', alignSelf: 'center', marginBottom: 30 },
  inputGroup: { marginBottom: 14, width: '100%' },
  label: { color: '#fff', fontSize: 14, marginBottom: 6, marginLeft: 6 },
  input: { width: '100%', backgroundColor: '#fff', borderRadius: 8, padding: 14, fontSize: 16 },
  button: { width: '100%', paddingVertical: 15, borderRadius: 9, marginVertical: 24, alignItems: 'center' },
  greenButton: { backgroundColor: '#22c55e' },
  whiteButton: { backgroundColor: '#fff' },
  buttonText: { fontSize: 18 },
  toggleText: { color: '#fff', fontSize: 15, textAlign: 'center' },
  toggleLink: { fontWeight: 'bold', color: '#22c55e' },
  rightSide: { flex: 1 },
